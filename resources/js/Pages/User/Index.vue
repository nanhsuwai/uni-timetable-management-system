...
let hasPermission = ref(checkPermissionComposable("user_manage"));
    ...
    ">
      <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
          <SectionTitleLineWithButton :icon="mdiShapePlus" title="Users Management">
            <!--  <BaseButton
              label="Add"
              color="contrast"
              small
          ...
          <div class="grid gap-2 py-6 border border-gray-500 px-3 mb-4 text-xs rounded sm:grid-cols-1 md:grid-cols-2">
            <div class="my-1">
              <label for="username" class="block mb-3 text-xs">Username</label>
              <TextInput v-model="filterUsername" id="username"
                class="w-full rounded border-gray-300 text-xs h-10 text-gray-700" type="text" autocomplete="off"
                placeholder="Enter username" />
            </div>
            <div class="my-1">
              <label for="course" class="block mb-3 text-xs">User Type</label>
              <VueMultiselect v-model="filterType" :options="props.userTypes" label="name">
              </VueMultiselect>
            </div>
            <div class='flex justify-start items-center'>
              <button @click="clear"
                class="py-2 px-4 rounded-lg hover:bg-slate-700 hover:text-slate-100 bg-slate-600 text-slate-50 focus:outline-none">
                Clear
              </button>
            </div>
          </div>
          ...
          <Modal :show="showPermissionModal" @close="closePermissionUpdateModal">
            <div class="p-4 max-h-[80vh] overflow-y-auto bg-white rounded-lg shadow-lg">
              <!-- Header -->
              <h2 class="text-lg font-bold text-gray-900 text-center mb-3">
                Update Permission
              </h2>
              <hr class="mb-3" />
              ...
              <div class="flex flex-wrap items-center justify-between mb-3">
                <h3 class="text-gray-700 text-sm">
                  Username:
                  <span class="font-bold">{{ selectedPermissionUser.name }}</span>
                </h3>
                <label class="flex items-center text-sm">
                  <input type="checkbox" v-model="allChecked" class="mr-2 accent-blue-600" />
                  Check All
                </label>
              </div>
              ...
              <div class="mt-4 flex justify-end space-x-2">
                <SecondaryButton @click="closePermissionUpdateModal" class="text-sm">
                  Cancel
                </SecondaryButton>
                <PrimaryButton :class="{ 'opacity-50': permissionForm.processing }"
                  :disabled="permissionForm.processing" @click="updatePermission" class="text-sm">
                  Update
                </PrimaryButton>
              </div>
            </div>
          ...
          <Modal :show="showUpdatePasswordModal" @close="closeUpdatePasswordModal">
            <div class="p-6">
              ...
              <div class="py-5">
                ...
                <TextInput id="email" type="text" disabled="true" autofocus="off" :value="selectedUser.email"
                  class="block w-full mt-1" autocomplete="off" />
              </div>
              <div class="py-5">
                <InputLabel for="password" value="Enter your password" />
                <TextInput id="password" v-model="updatePasswordForm.password" type="password" autofocus="off"
                  class="block w-full mt-1" placeholder="Enter a new password" autocomplete="off" />
              </div>
              <div>
                <InputLabel for="password_confirmation" value="Confirm your password" />
                <TextInput id="password_confirmation" v-model="updatePasswordForm.password_confirmation" type="password"
                  class="block w-full mt-1" placeholder="Confirm your password" autocomplete="off" />
              </div>
              ...
              <div class="mt-6 flex justify-end">
                ...
                <PrimaryButton class="ml-3" :class="{ 'opacity-25': updatePasswordForm.processing }"
                  :disabled="updatePasswordForm.processing" @click="updatePassword(selectedUser)">
                  Update
                </PrimaryButton>
              </div>
            </div>
          ...
          <Modal :show="confirmShowType" @close="closeTypeModal">
            <div class="p-6">
              ...
              <div>
                <select v-model="formType.user_type" class="mt-1 block w-full border-gray-300 rounded">
                  <option v-for="type_list in props.types" :value="type_list.code" :key="type_list.id"
                    class="capitalize my-3 py-4" :selected="$page.props.auth.user.user_type == type_list.code
                      ">
                    {{ type_list.name }}
                  </option>
                </select>
              ...
              <div class="mt-6 flex justify-end">
                ...
                <PrimaryButton class="ml-3" :class="{ 'opacity-25': formType.processing }"
                  :disabled="formType.processing" @click="editType">
                  Update
                </PrimaryButton>
              </div>
            </div>
            ...
            <div class="p-6">
              ...
              <div class="grid grid-cols-2 gap-2">
                <div class="mt-6">
                  ...
                  <div class="flex">
                    <select v-model="form.name_prefix" name="name_prefix" id="name_prefix"
                    ...
                    <TextInput id="myanmar_name" ref="myanmar_name" v-model="form.myanmar_name" type="text"
                      class="mt-1 block w-full" placeholder="Myanmar Name" />
                  </div>

                  <InputError :message="form.errors.myanmar_name" class="mt-2" />
                </div>
                <div class="mt-6">
                  <InputLabel for="english_name" value="အမည် (English)" />
                  <TextInput id="name" v-model="form.english_name" type="text" class="mt-1 block w-full"
              ...
              <div class="mt-6">
                <div class="mb-2">
                  <InputLabel for="nrc_number" value="နိုင်ငံသားစိစစ်ရေးကတ်ပြားအမှတ်" />
                </div>
                <div class="grid grid-cols-4 gap-1">
                  <select v-model="selectedRegion" name="nrc_region" id="nrc_region"
                    class="mt-1 block w-full border-gray-300 rounded">
                    <option value="">ပြန်နယ်/တိုင်း</option>
                    <option v-for="nrc_region in props.nrc_regions" :value="nrc_region.region_en">
                      {{ nrc_region.region_en }}
                    </option>
                  </select>
                  <select v-model="selectedTownship" name="nrc_township" id="nrc_township"
                    class="mt-1 block w-full border-gray-300 rounded">
                    <option value="">မြို့နယ်</option>
                    <option v-for="nrc_township in filterTownships" :value="nrc_township.township_en">
                      {{ nrc_township.township_en }}
                    </option>
                  </select>
                  <select v-model="selectedCitizen" name="nrc_citizen" id="nrc_citizen"
                    class="mt-1 block w-full border-gray-300 rounded">
                    <option value="">(နိင်)</option>
                    <option v-for="nrc_citizen in props.nrc_citizens" :value="nrc_citizen.citizen_en">
                      {{ nrc_citizen.citizen_en }}
                    </option>
                  </select>
                  <input v-model="selectedNumber" type="text" maxlength="6"
                    class="mt-1 block w-full border-gray-300 rounded" placeholder="123456" />
                </div>
              ...
              <div class="grid grid-cols-2 gap-2 items-center mt-6">
                <div>
                  ...
                  <TextInput id="name" v-model="form.username" type="text" class="mt-1 block w-full"
              ...
              <div class="grid grid-cols-2 gap-2">
                <div class="mt-6">
                  ...
                  <TextInput id="education" v-model="form.education" type="text" class="mt-1 block w-full"
                    placeholder="B.E (IT)" />
                ...
                <div class="mt-6">
                  ...
                  <div class="inline-flex">
                    <input type="radio" v-model="form.gender" value="female" class="mt-1 block px-2 mx-2" />
                    Female
                    <input type="radio" v-model="form.gender" value="male" class="mt-1 block px-2 mx-2" />
                    Male
                  </div>
              ...
              <div class="grid grid-cols-2 gap-2">
                <div class="mt-6">
                  ...
                  <TextInput id="employee_number" v-model="form.employee_number" type="text"
                    class="mt-1 block w-full border-gray-300 rounded" placeholder="Employee Number" />
                  <InputError :message="form.errors.employee_number" class="mt-2" />
                </div>
                <div class="mt-6">
                  <InputLabel for="employee_type" value="ဝန်ထမ်းအမှတ်" />
                  <select v-model="form.employee_type" name="employee_type" id="employee_type"
                    class="mt-1 block w-full border-gray-300 rounded">
              ...
              <div class="grid grid-cols-3 gap-2">
                ...
                <div class="mt-6">
                  <InputLabel for="office_phone" value="ရုံးဖုန်းနံပါတ်/ဖက်စ်" />
                  <TextInput id="office_phone" v-model="form.office_phone" type="text" class="mt-1 block w-full"
                    placeholder="09-xxx-xxx-xxxx" />
              ...
              <div class="grid grid-cols-3 gap-2">
                <div class="mt-6">
                  <InputLabel for="ministry" value="ဝန်ကြီးဌာန/အဖွဲ့အစည်း" class="mb-2" />
                  <select v-model="selectedMinistry" name="ministry" id="ministry"
                    class="mt-1 block w-full border-gray-300 rounded">
                    <option value="">
                      ဝန်ကြီးဌာန/အဖွဲ့အစည်း ရွေးချယ်ပါ...
                    </option>
                    <option v-for="ministry in props.ministries" :value="ministry.id">
                      {{ ministry.name }}
                    </option>
                  </select>
                ...
                <div class="mt-6">
                  <InputLabel for="department" value="ဦးစီးဌာန/လုပ်ငန်း" class="mb-2" />
                  <select v-model="selectedDepartment" name="department" id="department"
                    class="mt-1 block w-full border-gray-300 rounded">
                    <option value="">ဦးစီးဌာန/လုပ်ငန်း ရွေးချယ်ပါ...</option>
                    <option v-for="department in filterDepartments" :value="department.id">
                      {{ department.name }}
                    </option>
                  </select>
                </div>
                <div class="mt-6">
                  ...
                  <select v-model="form.position_id" name="position" id="position"
                    class="mt-1 block w-full border-gray-300 rounded">
                    <option value="">ရာထူး ရွေးချယ်ပါ...</option>
                    <option v-for="position in props.positions" :value="position.id">
                      {{ position.name }}
                    </option>
                  </select>
                </div>
              ...
              <div class="mt-6">
                <InputLabel for="job_description" value="လုပ်ငန်းတာဝန်" class="mb-2" />
                <textarea v-model="form.job_description" name="job_description" id="job_description" rows="4"
                  placeholder="လုပ်ငန်းတာဝန်" class="w-full rounded border-gray-300"></textarea>
              </div>
              <div class="mt-6">
                <InputLabel for="office_address" value="ရုံးလိပ်စာ" class="mb-2" />
                <textarea v-model="form.office_address" name="office_address" id="office_address" rows="4"
                  placeholder="ရုံးလိပ်စာ" class="w-full rounded border-gray-300"></textarea>
              </div>
              <div class="grid grid-cols-2 gap-2">
                <div class="mt-6">
                  <InputLabel for="profile_photo" value="ဓာတ်ပုံတင်ရန်" class="mb-2" />
                ...
                <div class="mt-6">
                  <InputLabel for="employee_card_photo" value="ဝန်ထမ်းကဒ်" class="mb-2" />
                  <input type="file" @input="form.employee_card_photo = $event.target.files[0]"
                    class="border-gray-300 rounded-lg py-1" />

                  <InputError :message="form.errors.employee_card_photo" class="mt-2" />
                </div>
              </div>
              <div class="mt-6">
                <InputLabel for="employee_card_photo" value="User Type" class="mb-2" />
                <select v-model="form.user_type" class="mt-1 block w-full border-1 border-gray-300 rounded">
                  <option v-for="type_list in props.types" :value="type_list.code" :key="type_list.id">
                    {{ type_list.name }}
                  </option>
                </select>
