...
watch(selectedSemester, (val) => (form.semester_id = val));
...
const getUniqueItems = (items, key) => {
  ...
  return items.filter((item) => {
    const val = item[key];
    // Use a composite key if necessary to ensure true uniqueness, 
    // e.g., if "Second Year" has a different ID/meaning for different years/programs.
    // For visual de-duplication based on name only:
...
const currentlySelectedLevelObject = computed(() =>
  props.levels.find((l) => l.id == selectedLevel.value)
);
/**
 * FIX: Computes levels filtered by program, then ensures names are unique 
 * to remove visual duplicates like 'Second Year', 'Second Year'.
 */
...
const filteredSections = computed(() => {
  // 1. Get the name of the currently selected level ID (e.g., "Second Year")
  const levelName = currentlySelectedLevelObject.value?.name;

  // If no program or level is selected, return all sections
  if (!selectedProgram.value || !levelName) return props.sections;

  // 2. Find all Level IDs that match the selected Name AND Program
  const matchingLevelIds = props.levels
    .filter(l => l.name === levelName && l.program_id == selectedProgram.value)
    .map(l => l.id);

  // 3. Filter sections to include those belonging to ANY of the matching Level IDs
...
const availableSemesters = computed(() => {
  const levelRecord = currentlySelectedLevelObject.value;
  if (!levelRecord?.semester_id) return null;
  const semester = props.semesters.find(s => s.id == levelRecord.semester_id);
  if (semester && semester.academic_year_id == selectedYear.value) {
    return semester;
  }
  return null;
});
          ...
          <div class="p-6 bg-white dark:bg-gray-800 rounded-lg">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <select v-model="selectedLevel"
                class="w-full border-gray-300 dark:border-gray-600 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 p-2.5">
                <option value="">Select Level</option>
                <option v-for="level in uniqueFilteredLevels" :key="level.id" :value="level.id">
                  {{ level.name }}
                </option>
              </select>
              <div>
              ...
              <div>
                ...
                <select v-model="form.semester_id"
                  ...
                  <option v-for="s in availableSemesters" :key="s.id" :value="s.id">{{ s.name }}</option>
                </select>
              </div>
